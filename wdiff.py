...
mytext = "Notes from the Underground"
...
def main():
    ...
    def load_img(path, shape, use_alpha_as_mask=False):
        ...
        if path.startswith('http://') or path.startswith('https://'):
            ...
            image = cv2_to_pil(img) #convert the image to PIL so you can use it that way.
    ...
    def render_animation(args, anim_args):
        ...
        # expand prompts out to per-frame
        settings_series = pd.Series([np.nan for a in range(anim_args.max_frames)])
        for i, setting in anim_settings.items():
            settings_series[int(i)] = setting
        settings_series = settings_series.ffill().bfill()
    ...
    fps = args.fps #@param {type:"number"}
